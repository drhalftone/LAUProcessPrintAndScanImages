...
photometric_palette_cmyk:
    ...
    if (t2p->pdf_defaultcompression == T2P_COMPRESS_JPEG) {
        if (t2p->pdf_colorspace & T2P_CS_PALETTE) {
            t2p->pdf_sample = t2p->pdf_sample | T2P_SAMPLE_REALIZE_PALETTE;
    ...
    if (TIFFGetField(input, TIFFTAG_WHITEPOINT, &xfloatp) != 0) {
        ...
        if (t2p->pdf_colorspace & T2P_CS_GRAY) {
            t2p->pdf_colorspace = t2p->pdf_colorspace | T2P_CS_CALGRAY;
        }
        if (t2p->pdf_colorspace & T2P_CS_RGB) {
            t2p->pdf_colorspace = t2p->pdf_colorspace | T2P_CS_CALRGB;
        }
    }
...
tsize_t t2p_write_pdf_catalog(T2P *t2p, TIFF *output)
{
    ...
    written += t2pWriteFile(output, (tdata_t) buffer,
                            qMin((size_t)buflen, sizeof(buffer) - 1));
    written += t2pWriteFile(output, (tdata_t) " 0 R \n", 6);
...
/*
  ...
  not seek.  See the example usage in the main() function.
    ...
    if(output->tif_seekproc != nullptr){
...
LAUTiff2PdfObject::LAUTiff2PdfObject(QString inputFileString, QString outputFileString, QObject *parent) : QObject(parent)
{
    T2P *t2p = t2p_init();
    if (t2p) {
        TIFF *input = TIFFOpen(inputFileString.toLatin1(), "r");
        if (input) {
            t2p->outputdisable = 0;
            t2p->outputfile = fopen(outputFileString.toLatin1(), "wb");
            if (t2p->outputfile == nullptr) {
                return;
            }

            TIFF *output = TIFFClientOpen(outputFileString.toLatin1(), "w", (thandle_t) t2p, t2p_readproc, t2p_writeproc, t2p_seekproc, t2p_closeproc, t2p_sizeproc, t2p_mapproc, t2p_unmapproc);
            if (output) {
                t2p_validate(t2p);
                t2pSeekFile(output, (toff_t) 0, SEEK_SET);
